  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-7 gap-3">
      {dates.map((date, idx) => {
        ...
        return (
          <div key={dateStr} className={`bg-white border rounded-2xl p-3 min-h-[160px] ${isToday ? 'border-brand' : 'border-border'}`}>
            <div className="mb-2">
            ...
            <div className="space-y-1.5">
              ...
              {dayAppointments.map((a) => (
                <button
                  ...
                  className={`w-full text-left text-[11px] px-2 py-1.5 rounded-lg font-semibold truncate ${
                    a.status === 'CANCELLED' ? 'bg-gray-100 text-gray-500 line-through' : 'bg-brand-soft text-brand-dark'
                  }`}
                >
                  {a.startTime} {a.clientFullName}
                </button>
              ))}
            </div>
          </div>
